...
\section{Styling}
  ...
  \subsection{Introduction}
    ...
    Styling determines how the content is layed out on the page.
    This is what makes layout usable on a variety of sreen sizes and the content user friendly.
...
\section{Interactive Web Frameworks}
  ...
  \subsection{Pros and cons for all JavaScript frameworks}
    ...
    It is currently being maintained by Google, and having the backing of such a big comapny means that it will be sticking around.
    ...
    \textbf{Pros:}
    React is maintained by Facebook, which has the same benifits as Angular's backing by Google.
    The framework will be actively developed by many people, the documentation will be good, and the product realiable.
    Since React was built more recently than some of the older frameworks, it has some respecteble benchmarks in terms of speed.\cite{SpeedReport}
    ...
    There are some features availabe in other frameworks like router or model management libraries that are not in React.
    A developer needs to be good at making decisions about what kind of frameworks should be added onto React to be able to do everything that other framworks can do.

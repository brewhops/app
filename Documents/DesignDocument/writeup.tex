...
\end{figure}
	...
	\subsection{Viewpoints}
		...
		\subsubsection{RESTful API}
			...
			/api/vats:
			\begin{itemize}
				\item GET: (queries VatInfo database table to get)
				\begin{itemize}
					\item VatID
					\item BrandID
					\item BATCH
					...
					\item Ferm\_To\_Vol
				\end{itemize}
				\item POST: (add a new Fermentation Vat to the database)
				\item /:id:
				\begin{itemize}
					\item GET: Query VatInfo for all batches produced in Vat
					\item POST: Add new batch to vat
					\item :id/:id\_batch: \\
						GET:
						\begin{itemize}
							\item Gets all rows of VatContent for BatchID
			...
			The user interfaces also need access to the Employees table in the database to authenticate users and regulate permission capabilities of various users. To do this, we expose a /users route on the url that allows the interfaces to pull back information on all users or specific users.
			/api/users:
			\begin{itemize}
		...
		\subsubsection{General User Interface}
			...
			\subsubsection{Maintainability}
				...
				Vat Monitoring & Vat Info \\
				\includegraphics[scale=.3]{images/mobile/vat_monitoring} & \includegraphics[scale=.3]{images/mobile/vat_info} \\
				\end{tabular}
